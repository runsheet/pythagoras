...
import { AgentConfiguration } from '../config/types';
import { ExecutionResult, PythangorasAgent } from './types';
import { ToolsManager } from '../tools/tools-manager';
import { GitHubMemoryManager } from '../memory/github-memory';
...
export class ToolsAgent implements PythangorasAgent {
  ...
  private _memory: GitHubMemoryManager;
  ...
  async run(objective: string): Promise<ExecutionResult> {
    ...
    const messages: BaseMessage[] = [
      new SystemMessage(
        `You are a helpful AI agent designed to assist with the following objective: ${objective}`
      ),
